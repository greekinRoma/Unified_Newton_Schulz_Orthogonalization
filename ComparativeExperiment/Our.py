...
class PolyModel(nn.Module):
    def __init__(self, N, weight_path=None, auto_load=True):
        """
        多项式模型类
...
class EfficientPolyModel(nn.Module):
    def __init__(self, N, weight_path=None, auto_load=True):
        """
        多项式模型类
        ...
        # 如果未指定权重路径，自动构建
        if weight_path is None:
            weight_path = os.path.join(output_dir, f'efficientpoly_model_N_{N}.pth')
        self.weight_path = weight_path
        
        # 自动加载权重（如果文件存在）
        if auto_load and os.path.exists(self.weight_path):
            self.load_weights(self.weight_path)
            self.c = math.exp(0.5) * (2**self.N) - torch.sum(torch.abs(self.a))
            print(f"模型 N={N}: 已从 '{self.weight_path}' 加载预训练权重")
        else:
            print(f"模型 N={N}: 使用随机初始化")
            train_model(self)  # 初始化时训练模型以获得合理权重
    ...
    def forward(self, x):
        y = x.clone()
        for k in range(1, self.N):
            y = y + torch.abs(self.a[k-1]) * x * (1 - x)**(2**(k-1))
        # 计算 b
        if self.c is None:
            c = math.exp(0.5) * (2**self.N) - torch.sum(torch.abs(self.a))
        else:
            c = self.c
        y = y + c * x * (1 - x)**(2**(self.N-1))
        return y
